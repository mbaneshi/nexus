...
**Context:** `fs` uses fully concrete types (no traits), `rusvel` uses 15 port traits. For a solo builder maintaining 8 crates, full hexagonal is overhead.

**Consequence:** Only `LlmPort` is abstract. Database, filesystem, and config access use concrete types passed directly. This can be expanded later if needed.
...
**Context:** Separate databases (one for files, one for configs) would complicate joins and backups. SQLite WAL mode handles concurrent reads well.
...
**Context:** Prevents circular dependencies and allows independent compilation. Proven pattern from `free-storage-app`.
...
**Context:** Alternatives: git repo for dotfiles, tar archives. SQLite BLOBs are simplest — no external dependencies, self-contained, easy to restore.
...
**Context:** Auto-detection would require heuristics and produce false positives. A curated registry of tools the user actually has (atuin, nvim, fish, git, etc.) is more reliable.

**Consequence:** Adding a new tool requires a code change. This is acceptable — the list changes rarely.
...
**Context:** User's primary LLM. Adding Ollama/OpenAI support can come later through the trait abstraction without changing any calling code.
...
**Context:** Both existing projects use this pattern. Prevents regressions from the start.

**Consequence:** Every PR runs the full pipeline. Releases triggered by git tags.
